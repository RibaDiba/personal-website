...
const ProjectModal = ({ project, onClose }: ProjectModalProps) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            {/* Modal Content */}
            <motion.div 
                className="relative z-10 w-full max-w-5xl aspect-[4/3] flex flex-col overflow-hidden rounded-3xl bg-[#57534E] text-white shadow-2xl"
            ...
            >
                {/* Header */}
                <div className="relative flex items-center justify-center border-b border-white/10 p-6 shrink-0">
                    <h2 className="text-3xl font-bold">{projectTitle}</h2>
                    <motion.button 
                        onClick={onClose}
                        className="absolute right-6 top-6 rounded-full p-1 hover:bg-white/10 transition-colors text-orange-200 cursor-pointer"
                ...
                {/* Body */}
                <div className="grid grid-cols-1 gap-8 p-8 md:grid-cols-2 flex-1 overflow-hidden">
                    {/* Left Column - Images */}
                    <div className="flex flex-col gap-6 px-4">
                        {/* Image 1 */}
                        <motion.div 
                            className="aspect-video w-full overflow-hidden rounded-xl bg-[#D9D9D9]"
                    ...
                    {/* Right Column - Text */}
                    <div className="flex flex-col gap-6 text-gray-200 font-semibold overflow-y-auto text-lg">
                        {(() => {
                ...
                {/* Footer */}
                <div className="flex items-center justify-between bg-[#D9D9D9] p-6 shrink-0">
                    <div className="flex gap-3">
                        {/* Placeholder Skills */}
                            { project.skills && project.skills.map((skill, i) => (
                                <motion.span 
                                    key={i} 
                                    className="rounded-full bg-[#57534E] px-4 py-2 text-sm font-semibold text-white shadow-sm"
                                    whileHover={{ scale: 1.1 }} 
                                    transition={{ type: "spring", stiffness: 300 }}
                                >
                                    {skill}
                    ...
                    {project.ghLink && (
                        <a href={project.ghLink}>
                            <motion.button 
                                className="rounded-full bg-[#57534E] px-6 py-2 font-bold text-white shadow-sm hover:bg-[#44403c] transition-colors cursor-pointer"
                                whileHover={{ scale: 1.1 }} 
                                transition={{ type: "spring", stiffness: 300 }}
                            >
                                &lt;Code /&gt;
